...
- Users can perform CRUD operations on posts as well as like, comment, and share other people's posts. In addition to that, a user can follow another user.
- Implemented Authentication and Real-time updates of likes and comments using Firebase Auth and Firestore.
- Integrated TanstackQuery for caching server-side data and deduping multiple requests for the same data into a single one.
